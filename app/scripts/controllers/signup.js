...
angular.module('boltApp.controllers.Signup', [])
	.controller('SignupCtrl', function($scope, $rootScope, $q, $http, $cookieStore, $window, $document, $location, $modal, $timeout, getCities, getCityId, RestApi, gettextCatalog) {
		...
		$scope.cityChange = function() {
			var selectedCity = _.findWhere($scope.cities, {
				id: $scope.order.cityId
			});
...
"name": "Max Musterman"
		...
		$scope.changeType = function() {
			...
			$scope.overview = {
				card: _.findWhere($scope.cards, {
					code: $scope.order.type
				}).displayName,
				price: _.findWhere($scope.cards, {
					code: $scope.order.type
				}).monthlyPrice
			};
